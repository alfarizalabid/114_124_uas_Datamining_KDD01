...
# PREDICTION 
    ...
    # --- Mapping kota representatif California ---
    KOTA_CALIFORNIA = {
    ...
    with st.expander("Lokasi Properti", expanded=True):
        ...
        # Ambil nilai lat/lon & ocean_proximity dari mapping
        ...
        # Tampilkan info lokasi yang dipilih (read-only, hanya informatif)
    ...
    if st.button("Proses Prediksi"):
        with st.spinner("Memproses data..."):
            try:
                # Step 1: Buat DataFrame fitur numerik sesuai urutan fit scaler
                ...
                # Step 2: Scale fitur numerik
                scaled_arr = scaler.transform(input_num)
                scaled_df = pd.DataFrame(scaled_arr, columns=scaler.feature_names_in_)

                # Step 3: Prediksi cluster dari KMeans (input = 8 fitur numerik ter-scale)
                klaster_prediksi = int(kmeans.predict(scaled_df)[0])

                # Step 4: One-Hot Encoding ocean_proximity secara manual
                # Urutan kolom OHE harus sama persis dengan saat pd.get_dummies() di training
                ohe_cols = [
                ...
                # Step 5: Gabungkan semua fitur sesuai urutan model.feature_names_in_:
                # [longitude, latitude, ..., median_income,
                #  ocean_proximity_<1H OCEAN, ..., ocean_proximity_NEAR OCEAN, cluster]
...
# VISUALIZATION
    ...
    # 1. Buat Dropdown untuk memilih jenis grafik
    ...
    # 2. Logika untuk Peta Persebaran
    ...
    # 3. Logika untuk Histogram Distribusi
    ...
    # 4. Logika untuk Scatter Plot Dinamis (Bisa diubah-ubah sumbunya)
        ...
        # Ambil hanya kolom yang berisi angka (hindari kolom teks seperti ocean_proximity)
        kolom_numerik = df.select_dtypes(include=np.number).columns
        
        col_x, col_y = st.columns(2)
        with col_x:
            # Default index 7 adalah median_income
            sumbu_x = st.selectbox("Pilih Sumbu X:", kolom_numerik, index=7) 
        with col_y:
            # Default index 8 adalah median_house_value
